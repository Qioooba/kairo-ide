# Milestones — Kairo IDE (ARCHIVED)

> **ARCHIVED: 2026-07-19**
> This document has been replaced by the new [MILESTONES.md](../MILESTONES.md) truth matrix.
> The original document had contradictory statements — the top claimed "prototype phase"
> but the bottom claimed "honestly working." This copy is preserved for historical reference only.
> See [NEXT_ITERATION_BASELINE.md](../progress/NEXT_ITERATION_BASELINE.md) for the current Wave 0 baseline.

---

> **⚠️ 当前状态：原型阶段 (Prototype Phase)**
>
> 本文档描述的是原型阶段的实际完成情况。项目已进入架构重审阶段，
> 新的实施基线为 **[REARCHITECTURE_AND_IMPLEMENTATION_PLAN.md](./REARCHITECTURE_AND_IMPLEMENTATION_PLAN.md)**。
> 该文档基于交叉评审发现的问题（Go Agent 无法编译、事件流未实现、
> 远程模式无认证、Desktop/Server 为壳、JDT LS 进程生命周期错误、
> 主业务链未闭环等），提出了推倒重做业务编排层和 API 契约的方案。
> 后续所有开发工作应以 REARCHITECTURE_AND_IMPLEMENTATION_PLAN.md
> 为唯一实施基线，本文件仅作为历史参考。
...
- [~] `apps/desktop` Electron entry (Windows / macOS) — shell only
- [~] `apps/server` long-running server entry (Linux) — shell only
...
- [x] HTTP server with middleware (request ID, recovery, structured
      logging, audit)
- [~] WebSocket event bus endpoint (handler stub only; full WS
...
- [~] DAP bridge interface (in `/api/v1/servers/{id}/debug`) —
      interface only, no real implementation
...
- [~] Auth endpoint (`/api/v1/auth/login`); stub accepts any
      non-empty pair. Real Argon2id is M7.1.
- [ ] Session cookie + CSRF: not yet implemented (M7.1)
- [~] WebSocket auth via subprotocol: handler stub only
      (the WS upgrade is not implemented)
...
- [~] TLS termination: config fields exist; real cert + key
      loading is M7.1
- [~] `apps/server` reverse-proxy entry that talks to the
      in-process Runtime Agent — shell only

## M8 — Plugin + Polish + Release

- [~] Three-layer plugin loader (interfaces defined in
      `api/services.go`; the loader is scaffolded only)
- [~] `@kairo/plugin-scaffold` template (README only; not yet a
      `pnpm create` template)
- [~] Built-in `org.kairo.tomcat6` plugin: the abstraction is
      in place (`ServerRuntimeProvider`); the actual plugin
      binary is not implemented
- [~] Plugin manifest schema + JSON Schema validation
