...
export class SearchEverywhereFilesProvider implements SearchEverywhereProvider {
  ...
  async search(query: string, signal: AbortSignal, limit: number): Promise<SearchEverywhereItem[]> {
    const context = this.workspace.requireContext(); const root = URI.fromFilePath(context.workspaceRoot);
    const queue: URI[] = [root]; const results: SearchEverywhereItem[] = []; let visited = 0;
    while (queue.length && results.length < limit && visited < 5000) {
      ...
      for (const child of stat.children ?? []) {
        ...
        if (child.isFile && fuzzyScore(query, child.resource.path.toString()) !== undefined) results.push({ id: `file:${child.resource}`, category: 'files', label: child.resource.path.base, detail: root.relative(child.resource)?.toString(), uri: child.resource.toString() });
