...
function makeRecorder(optsOrOutDir) {
  ...
  const defaults = {
    ...
    // Prefer explicit opts.round, else KAIRO_QA_ROUND env (Round 10 GAP-7).
    round: opts.round != null ? opts.round : Number(process.env.KAIRO_QA_ROUND || 1),
  };
...
/** Status bar shows an active project (zh or en). Used by import + GAP-1/QA-011. */
function statusBarHasProject(bar) {
  const t = String(bar || '');
  return (
    /(项目|Project)[：:].+/i.test(t) &&
    !/未导入|not imported|\(无工作区\)|\(no workspace\)/i.test(t)
  );
}
...
/** Reconnect agent if status bar shows disconnected/closed (A2 Server:disconnected root cause). */
async function ensureAgentHealthy(page) {
  let agent = await waitAgent(page, 8_000);
  if (agent.ok) return agent;
  try {
    await execCommand(page, 'kairo.agent.reconnect');
  } catch (_) {
    try {
      await runPalette(page, 'Reconnect Agent');
    } catch (__) {}
  }
  await sleep(1500);
  agent = await waitAgent(page, 45_000);
  return agent;
}
...
async function waitJdtReady(page, timeoutMs = 120_000) {
  ...
  while (Date.now() - t0 < timeoutMs) {
    ...
    // After ≥75s with a Java editor open, accept even if command registry
    // hasn't exposed java.* yet (Round 10 scenario A.2 flake).
    if (!busy && probe.hasEditor && probe.javaContext && Date.now() - t0 >= 75_000) {
      return { ok: true, bar, waitedMs: Date.now() - t0, soft: true, via: 'java-editor' };
    }
    await sleep(2000);
  }
  return { ok: false, bar: await statusBarText(page), waitedMs: Date.now() - t0 };
...
async function launchDesktop({ outDir, workspaceArg, settleMs }) {
  ...
  // Default settle helps flaky UI trains; cold-start bench passes settleMs=0
  // so the metric matches “to status bar ready” (OPT-001 / §6.4).
  const settle = settleMs != null ? Number(settleMs) : 3000;
