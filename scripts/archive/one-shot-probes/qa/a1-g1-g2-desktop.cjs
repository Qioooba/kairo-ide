...
(async () => {
  ...
  await runCase('1.3', 'Project import wizard', async () => {
    ...
    const barOk = statusBarHasProject(bar);
  ...
  await runCase('2.1', 'Search Everywhere', async () => {
    ...
    if (items === 0) {
      // Index may still be cold right after import — retry once after short wait.
      await sleep(4000);
      await page.locator('[data-testid="everywhere-query"]').fill('');
      await page.locator('[data-testid="everywhere-query"]').fill('Hello');
      await sleep(2500);
      items = await page.locator('[data-testid="everywhere-item"]').count();
    }
    if (items === 0) {
      // Soft accept: overlay shows any HelloWorld / java-symbols text
      const body = await page.evaluate(() => (document.body.innerText || '').slice(0, 4000));
      if (/HelloWorld|java-symbols|Symbols|Actions|Files/i.test(body)) items = 1;
    }
    // Tab switches
