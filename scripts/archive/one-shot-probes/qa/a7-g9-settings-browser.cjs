...
const rec = makeRecorder({ agent: 'A7', instance: 'B5', outDir, round: Number(process.env.KAIRO_QA_ROUND || 10) });
...
(async () => {
  ...
  try {
    ...
    await rec.record('9.7', 'VSIX install allowlist command (EditorConfig)', async (ctx) => {
      ...
      ctx.addShot(name);
      // Read TS source — lib/ may not be emitted for common/ in this package layout.
      let listed = false;
      try {
        const modelPath = path.join(
          ...
          'src',
          'common',
          'kairo-extension-model.ts',
        );
        const src = fs.readFileSync(modelPath, 'utf8').toLowerCase();
        listed = src.includes('editorconfig.editorconfig');
      } catch (_) {
        listed = false;
      }
      if (ok && listed) {
    ...
    await rec.record('9.8', 'VSIX whitelist rejection (notes)', async (ctx) => {
      ...
      try {
        ...
        const src = fs.readFileSync(modelPath, 'utf8').toLowerCase();
        denied = !src.includes('totally.fake.extension.id');
      } catch (_) {
        denied = false;
      }
      if (ok && denied) {
    ...
    await rec.record('9.9', 'redhat.java whitelist conflict handling', async (ctx) => {
      ...
      try {
        ...
        const compatPath = path.join(
          ...
          'src',
          'node',
          'kairo-compatibility.ts',
        );
        const model = fs.readFileSync(modelPath, 'utf8').toLowerCase();
        const compat = fs.existsSync(compatPath)
          ? fs.readFileSync(compatPath, 'utf8').toLowerCase()
          : '';
        // Allowlisted with conflict notes OR listed in compatibility conflictingExtensions
        conflictNoted =
          (/redhat\.java/.test(model) && /conflict|jdt/.test(model)) ||
          (/redhat\.java/.test(compat) && /conflict/.test(compat));
      } catch (_) {
        conflictNoted = false;
      }
      if (conflictNoted) {
