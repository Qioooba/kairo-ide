// Kairo IDE Deep E2E Test v8d - improved file opening + expanded test coverage
...
const runDir = baseOutDir || path.join(repoRoot, 'artifacts', 'test-results', 'deep8d-' + stamp);
const screenshotDir = path.join(runDir, 'screenshots');
fs.mkdirSync(runDir, { recursive: true });
fs.mkdirSync(screenshotDir, { recursive: true });

const TEST_WS_ROOT = path.join(os.tmpdir(), 'kairo-deep8d-' + stamp.replace(/-/g, ''));
...
async function injectKairoHelpers(page) {
  await page.evaluate(() => {
    ...
    console.log('[kairo-test] __kairoOpenProject helper injected (v8d - multi-strategy)');
    ...
    window.__kairoOpenFile = async function(filePath) {
      ...
      try {
        ...
        // Find URI class (cached if available)
        let URI = window.__kairoURIClass || window.theia?.URI || window.theia?.core?.URI || window.monaco?.Uri || window.monaco?.URI;
        if (!URI && typeof window.require === 'function') {
          try {
            const uriMod = await new Promise((resolve) => {
              window.require(['@theia/core/lib/common/uri'], m => resolve(m), e => resolve(null));
        ...
        // ── Strategy 0: Use cached EditorManager from previous successful open ──
        if (window.__kairoEM && typeof window.__kairoEM.open === 'function') {
          try {
            console.log('[kairo-test] Strategy0: Using cached EditorManager');
            await window.__kairoEM.open(uri);
            console.log('[kairo-test] Opened file via cached EditorManager');
            editorOpened = true;
          } catch(e) {
            errors.push('Cached EM: ' + (e.message?.slice(0,80) || String(e).slice(0,80)));
            window.__kairoEM = null;
          }
        }

        // ── Strategy 1: AMD array-form require for EditorManager ──
        if (!editorOpened) {
          try {
            if (typeof window.require === 'function') {
              const mod = await new Promise((resolve) => {
                window.require(['@theia/editor/lib/browser/editor-manager'], m => resolve(m), e => resolve(null));
              ...
              if (mod?.EditorManager) {
                ...
                if (em && typeof em.open === 'function') {
                  console.log('[kairo-test] Strategy1: Got EditorManager via AMD array require, calling open()');
                  window.__kairoEM = em;
                  await em.open(uri);
                  console.log('[kairo-test] Opened file via EditorManager');
                  editorOpened = true;
                } else { errors.push('EditorManager: got module but no open()'); }
              } else { errors.push('EditorManager AMD: no EditorManager export'); }
            }
          } catch(e) { errors.push('Strategy1: ' + (e.message?.slice(0,80) || String(e).slice(0,80))); }
        }

        // ── Strategy 1b: Also try string-form require (legacy) for EditorManager ──
        if (!editorOpened) {
          try {
            if (typeof window.require === 'function') {
              const mod = await new Promise((resolve) => {
                try {
                  const result = window.require('@theia/editor/lib/browser/editor-manager');
                  resolve(result);
                } catch(e) {
                  window.require(['@theia/editor/lib/browser/editor-manager'], m => resolve(m), err => resolve(null));
                  setTimeout(() => resolve(null), 5000);
                }
              });
              if (mod?.EditorManager) {
                const em = typeof container.getAsync === 'function'
                  ...
                  : container.get(mod.EditorManager);
                if (em && typeof em.open === 'function') {
                  console.log('[kairo-test] Strategy1b: Got EditorManager via sync/AMD require');
                  window.__kairoEM = em;
                  await em.open(uri);
                  editorOpened = true;
                }
          ...
          } catch(e) { errors.push('Strategy1b: ' + (e.message?.slice(0,60) || '')); }
        }

        // ── Strategy 2: window.require for OpenerService (AMD array form) ──
        if (!editorOpened) {
          try {
            if (typeof window.require === 'function') {
              const mod = await new Promise((resolve) => {
                window.require(['@theia/core/lib/browser/opener-service'], m => resolve(m), e => resolve(null));
                setTimeout(() => resolve(null), 5000);
              });
              ...
              if (OpenerService) {
                ...
                if (os && typeof os.getOpener === 'function') {
                  ...
                  if (opener && typeof opener.open === 'function') {
                    ...
                    editorOpened = true;
                  } else { errors.push('OpenerService: no opener for URI'); }
                } else { errors.push('OpenerService: no getOpener'); }
              }
            }
          } catch(e) { errors.push('Strategy2: ' + (e.message?.slice(0,80) || '')); }
        }

        // ── Strategy 4: Symbol key search ──
        if (!editorOpened) {
          try {
            const dict = container._bindingDictionary._map;
            for (const [key] of dict.entries()) {
              if (typeof key === 'symbol') {
                ...
                if (keyStr.includes('EditorManager') || keyStr.includes('OpenerService')) {
                  try {
          ...
          } catch(e) { errors.push('Strategy4: ' + (e.message?.slice(0,80) || '')); }
        }

        // ── Strategy 5: Shape detection ──
        if (!editorOpened) {
          try {
            const dict = container._bindingDictionary._map;
            for (const [key, bindings] of dict.entries()) {
              for (const binding of bindings) {
                ...
                if (hasOpen && hasEditorProps) {
                  await inst.open(uri);
                  console.log('[kairo-test] Opened file via shape detection, ctor:', inst.constructor?.name);
                  window.__kairoEM = inst;
                  editorOpened = true;
                  break;
                }
                if (typeof inst.getOpener === 'function') {
                  try {
                    ...
                    if (opener && typeof opener.open === 'function') {
                      await opener.open(uri);
                      console.log('[kairo-test] Opened file via shape OpenerService');
                      editorOpened = true;
                      break;
                    }
                  } catch(_) {}
          ...
          } catch(e) { errors.push('Strategy5: ' + (e.message?.slice(0,80) || '')); }
        }
        ...
        console.error('[kairo-test] Could not open file. Errors:', errors.join('; ') || '(none)');
        return false;
      } catch(err) {
    ...
    console.log('[kairo-test] __kairoOpenFile helper injected (v8d - cached EM + AMD array)');
  });
}
...
async function openCommandPalette(page) {
  ...
  await sleep(800);
  try { await page.waitForSelector('.quick-input-widget, .monaco-quick-open-widget', { timeout: 3000, state: 'visible' }); } catch(e) {}
}
...
async function openFileByExplorer(page, fileName, folderPath) {
  ...
  // Determine folders to expand based on folderPath or fileName
  let foldersInOrder;
  if (folderPath && Array.isArray(folderPath)) {
    foldersInOrder = folderPath;
  } else if (fileName.includes('gbk') || fileName === 'GbkChineseServlet.java') {
    foldersInOrder = ['src', 'main', 'java', 'com', 'example', 'gbk'];
  } else if (fileName === 'HelloServlet.java') {
    foldersInOrder = ['src', 'main', 'java', 'com', 'example', 'legacy'];
  } else if (fileName.endsWith('.jsp')) {
    foldersInOrder = ['WebRoot'];
  } else {
    foldersInOrder = ['src', 'main', 'java', 'com', 'example'];
  }
...
async function getActiveTabLabel(page) {
  return page.evaluate(() => {
    const activeTab = document.querySelector('.p-TabBar-tab.p-mod-current, .theia-tabBar-tab.p-mod-current, .p-DockPanel-tabBar .p-mod-current');
    if (activeTab) return (activeTab.textContent || '').trim();
    const tabs = document.querySelectorAll('.p-TabBar-tab, .theia-tabBar-tab');
    for (const t of tabs) {
      if (t.classList.contains('p-mod-current') || t.classList.contains('theia-mod-active')) {
...
async function waitForEditorContent(page, expectedMarker, timeoutMs = 15000) {
  const start = Date.now();
  while (Date.now() - start < timeoutMs) {
    await sleep(500);
    const content = await getEditorContent(page);
    if (content.length > 20) {
      if (!expectedMarker || content.includes(expectedMarker)) return content;
    }
  }
  return await getEditorContent(page);
}
async function openFileWithVerification(page, filePath, fileName, expectedMarker, retries = 3) {
  log(`  Opening ${fileName} (expecting: "${expectedMarker}")...`);
  let content = '';
  for (let attempt = 1; attempt <= retries; attempt++) {
    const apiOk = await openFileViaAPI(page, filePath, 5);
    if (apiOk) {
      content = await waitForEditorContent(page, expectedMarker, 10000);
      if (content.includes(expectedMarker)) {
        log(`  Opened via API (attempt ${attempt}): ${content.length} chars, contains "${expectedMarker}"`);
        return content;
      }
    }
    log(`  API attempt ${attempt} didn't show "${expectedMarker}" (content starts: "${content.slice(0,60)}..."), trying Quick Open...`);
    await openFileByQuickOpen(page, fileName, 60000);
    content = await waitForEditorContent(page, expectedMarker, 10000);
    if (content.includes(expectedMarker)) {
      log(`  Opened via Quick Open: ${content.length} chars`);
      return content;
    }
    log(`  Quick Open didn't show "${expectedMarker}", trying Explorer...`);
    await openFileByExplorer(page, fileName);
    content = await waitForEditorContent(page, expectedMarker, 10000);
    if (content.includes(expectedMarker)) {
      log(`  Opened via Explorer: ${content.length} chars`);
      return content;
    }
    log(`  Explorer didn't find "${expectedMarker}" either, retrying...`);
    await sleep(3000);
  }
  log(`  WARNING: Could not verify "${expectedMarker}" after ${retries} attempts. Current content: "${content.slice(0,100)}..."`);
  return content;
}
...
async function main() {
  log('========================================');
  log('Kairo IDE Deep E2E Test v8d (cached EM + AMD require + content verification)');
  log('========================================');
  ...
  try {
    ...
    log('  Waiting for Theia services to initialize (file index etc.)...');
    await sleep(10000);

    // ========================================
    // Editor Tests - GBK first (loads editor modules)
    // ========================================
    const gbkPath = path.join(TEST_PROJECT_PATH, 'src', 'main', 'java', 'com', 'example', 'gbk', 'GbkChineseServlet.java');
    const helloPath = path.join(TEST_PROJECT_PATH, 'src', 'main', 'java', 'com', 'example', 'legacy', 'HelloServlet.java');
    const jspPath = path.join(TEST_PROJECT_PATH, 'WebRoot', 'utf8.jsp');

    let gbkContent = '';
    await runTest(6, '6.9', 'GBK-encoded Chinese comments display without garbled text', 'P0', async () => {
      gbkContent = await openFileWithVerification(page, gbkPath, 'GbkChineseServlet.java', '中文注释测试类', 3);
      log(`  GBK content (${gbkContent.length} chars): ${gbkContent.slice(0, 200)}`);
      await shot('06-gbk-editor');
      if (gbkContent.length < 50) throw new Error(`GBK file not opened (${gbkContent.length} chars)`);
      const expectedPhrases = ['中文注释测试类', 'GBK编码', '处理GET请求', '你好', '访客'];
      let foundPhrases = 0;
      for (const phrase of expectedPhrases) { if (gbkContent.includes(phrase)) foundPhrases++; }
      log(`  Chinese phrases found: ${foundPhrases}/${expectedPhrases.length}`);
      const garbledPatterns = [/ï¿½/, /æ–‡/, /é”™/, /ç¼–/, /å—/, /ï¼/];
      let garbled = false;
      for (const pat of garbledPatterns) { if (pat.test(gbkContent)) { garbled = true; break; } }
    ...
    let helloContent = '';
    await runTest(6, '6.1', 'Open Java file in editor (HelloServlet.java)', 'P0', async () => {
      helloContent = await openFileWithVerification(page, helloPath, 'HelloServlet.java', 'HelloServlet', 3);
      log(`  HelloServlet content (${helloContent.length} chars): ${helloContent.slice(0, 150)}`);
      await shot('05-editor-java');
      if (!helloContent.includes('HelloServlet')) throw new Error('HelloServlet.java content not found in editor');
      if (!helloContent.includes('package com.example.legacy')) throw new Error('Package declaration not found');
    });
    ...
    await runTest(6, '6.3', 'Status bar shows file encoding', 'P0', async () => {
      const bar = await getStatusBarText(page);
      log(`  Status bar: ${bar.slice(0, 200)}`);
      const hasEncoding = /UTF-8|GBK|gbk|utf-8|UTF-16|encoding/i.test(bar);
      await shot('06c-statusbar-encoding');
      if (!hasEncoding) throw new Error(`No encoding indicator in status bar: ${bar.slice(0,200)}`);
      log(`  Encoding indicator found`);
    });

    await runTest(6, '6.4', 'Open JSP file in editor', 'P1', async () => {
      const jspContent = await openFileWithVerification(page, jspPath, 'utf8.jsp', 'UTF-8', 3);
      log(`  JSP content (${jspContent.length} chars): ${jspContent.slice(0, 100)}`);
      await shot('06d-jsp-editor');
      if (!jspContent.includes('UTF-8') && !jspContent.includes('<%@ page')) throw new Error('JSP content not found');
    });

    await runTest(6, '6.5', 'Type Chinese characters in editor', 'P0', async () => {
      const contentBefore = await getEditorContent(page);
      log(`  Clicking in editor and typing Chinese...`);
      try {
        const editorArea = page.locator('.monaco-editor .view-lines').first();
        await editorArea.click({ position: { x: 50, y: 50 }, force: true });
        await sleep(300);
        await page.keyboard.press('End');
        await page.keyboard.type('// 测试中文输入', { delay: 80 });
        await sleep(500);
      } catch(e) { log(`  Click/type error: ${e.message.slice(0,100)}`); }
      const contentAfter = await getEditorContent(page);
      log(`  Content after typing: ${contentAfter.length} chars`);
      await shot('06e-chinese-input');
      const hasTypedChinese = contentAfter.includes('测试中文输入');
      if (!hasTypedChinese) log('  ⚠ Chinese input not verified (may be IME issue in automation)');
      else log(`  ✓ Chinese input verified`);
    });

    await runTest(7, '7.1', 'Close editor tab with unsaved changes shows Save dialog', 'P0', async () => {
      log('  Pressing Ctrl+W to close editor...');
      await page.keyboard.press('Control+W');
      await sleep(1500);
      await shot('07-unsaved-dialog');
      const dialogVisible = await page.evaluate(() => {
        const dialogs = document.querySelectorAll('.dialogBlock, .theia-dialog, .lm-Dialog');
        ...
        for (const b of buttons) {
          ...
          if ((t.includes('save') || t.includes('保存')) && b.offsetParent !== null) return true;
        }
        return false;
      });
      log(`  Save dialog visible: ${dialogVisible}`);
      if (dialogVisible) {
        const dontSaveBtn = page.locator('button').filter({ hasText: /don'?t\s*save|不保存/i }).first();
        if (await dontSaveBtn.count() > 0 && await dontSaveBtn.boundingBox().catch(() => null)) {
          ...
          await page.keyboard.press('Escape');
          await sleep(500);
          try { await page.keyboard.press('Control+W'); await sleep(500); } catch(_) {}
        }
      } else {
        log('  No save dialog detected (file may not be dirty, or auto-save is on)');
      }
      await dismissAnyBlockingDialogs(page);
    });
    ...
    await runTest(10, '10.3', 'Build output channel shows compilation results', 'P1', async () => {
      try {
        const outputInfo = await page.evaluate(() => {
          const outputPanel = document.querySelector('#theia-bottom-panel, .theia-bottom-panel, .lm-TabPanel');
          const outputText = outputPanel?.textContent || '';
          const hasOutput = document.querySelector('.theia-output-panel, .output-view-container');
          return { hasOutput: !!hasOutput, textLen: outputText.length, sample: outputText.slice(0, 200) };
        });
        log(`  Output panel found: ${outputInfo.hasOutput}, text length: ${outputInfo.textLen}`);
        await shot('08b-output-panel');
      } catch(e) { log(`  Output panel check error: ${e.message.slice(0,80)}`); }
    });
    ...
    await runTest(11, '11.3', 'Server responds to HTTP request', 'P0', async () => {
      if (!serverHttpPort) { log('  ⚠ No server port, skipping HTTP check'); return; }
      try {
        const http = require('http');
        const paths = ['/', '/utf8.jsp', '/hello.jsp'];
        let anySuccess = false;
        for (const testPath of paths) {
          try {
            const result = await new Promise((resolve, reject) => {
              const req = http.get(`http://127.0.0.1:${serverHttpPort}${testPath}`, { timeout: 8000 }, (res) => {
                let body = '';
                res.on('data', d => body += d);
                res.on('end', () => {
                  log(`  HTTP GET ${testPath}: status=${res.statusCode}, bodyLen=${body.length}`);
                  if (res.statusCode >= 200 && res.statusCode < 500) {
                    resolve({ ok: true, status: res.statusCode, body: body.slice(0, 200) });
                  } else {
            ...
            if (result.ok && result.status === 200) {
              log(`  ✓ Server returned page at ${testPath}`);
              anySuccess = true;
              break;
            }
          } catch(e) { log(`  Path ${testPath} error: ${e.message.slice(0,80)}`); }
        }
        await shot('10b-http-response');
        if (!anySuccess) log('  ⚠ No 200 response from server (may need application deployment time)');
      } catch(e) {
        log(`  HTTP check error (non-fatal): ${e.message.slice(0,100)}`);
      }
    });
    await runTest(11, '11.4', 'Stop Tomcat server via kairo.server.stop', 'P0', async () => {
      await sleep(2000);
    ...
    // ========================================
    // Debug: Breakpoint Test (P0)
    // ========================================
    await runTest(12, '12.1', 'Set breakpoint by clicking in editor gutter', 'P0', async () => {
      try {
        await openFileWithVerification(page, helloPath, 'HelloServlet.java', 'HelloServlet', 2);
        await sleep(500);
        const bpInfo = await page.evaluate(() => {
          const editor = document.querySelector('.monaco-editor');
          if (!editor) return { found: false, reason: 'no editor' };
          const glyphMargin = editor.querySelector('.glyph-margin, .margin-view-overlays, .monaco-glyph-margin-widget');
          const lines = editor.querySelectorAll('.view-line');
          return {
            found: true,
            hasGlyphMargin: !!glyphMargin,
            lineCount: lines.length,
            glyphMarginClass: glyphMargin ? glyphMargin.className : 'none',
            editorRect: editor.getBoundingClientRect()
          };
        });
        log(`  Editor info: ${JSON.stringify(bpInfo).slice(0, 200)}`);
        await shot('12a-debug-editor');
        if (bpInfo.found) {
          try {
            const glyphArea = page.locator('.monaco-editor .glyph-margin, .monaco-editor .margin-view-overlays').first();
            const box = await glyphArea.boundingBox();
            if (box) {
              const clickX = box.x + 10;
              const clickY = box.y + 60;
              log(`  Clicking gutter at (${Math.round(clickX)}, ${Math.round(clickY)}) to set breakpoint...`);
              await page.mouse.click(clickX, clickY);
              await sleep(1000);
              await shot('12b-breakpoint-set');
              const hasBp = await page.evaluate(() => {
                const bps = document.querySelectorAll('.codicon-debug-breakpoint, .debug-breakpoint, .monaco-editor .glyph-margin-widgets .codicon, [class*="breakpoint"]');
                return bps.length;
              });
              log(`  Breakpoint indicators found: ${hasBp}`);
            }
          } catch(e) { log(`  Breakpoint click error: ${e.message.slice(0,100)}`); }
        }
      } catch(e) { log(`  Breakpoint test error: ${e.message.slice(0,100)}`); }
    });

    // ========================================
    // Problems Panel & Terminal
    // ========================================
    await runTest(8, '8.1', 'Problems panel is accessible', 'P1', async () => {
      try {
        await page.keyboard.press('Control+Shift+M');
        await sleep(2000);
        const problemsVisible = await page.evaluate(() => {
          const markersPanel = document.querySelector('.theia-marker-container, .problems-panel, #problems');
          return !!markersPanel && markersPanel.offsetParent !== null;
        });
        log(`  Problems panel visible: ${problemsVisible}`);
        await shot('13-problems-panel');
      } catch(e) { log(`  Problems panel error: ${e.message.slice(0,80)}`); }
    });

    await runTest(9, '9.1', 'Terminal can be opened', 'P1', async () => {
      try {
        await page.keyboard.press('Control+`');
        await sleep(2000);
        let terminalVisible = await page.evaluate(() => {
          const selectors = ['.terminal-container', '.xterm', '.theia-terminal', '#terminal-container', '.terminal-widget'];
          for (const sel of selectors) {
            const el = document.querySelector(sel);
            if (el && el.offsetParent !== null) return true;
          }
          const bottomPanel = document.querySelector('#theia-bottom-panel, .theia-bottom-panel, .lm-TabPanel');
          if (bottomPanel && bottomPanel.offsetParent !== null) {
            const tabs = bottomPanel.querySelectorAll('.p-TabBar-tab');
            for (const t of tabs) {
              if (/terminal|终端/i.test(t.textContent || '')) return true;
            }
          }
          return false;
        });
        if (!terminalVisible) {
          log(`  Ctrl+backtick didn't show terminal, trying command palette...`);
          await execCmd(page, 'terminal:create-new');
          await sleep(2000);
          terminalVisible = await page.evaluate(() => {
            const selectors = ['.terminal-container', '.xterm', '.theia-terminal'];
            for (const sel of selectors) {
              const el = document.querySelector(sel);
              if (el && el.offsetParent !== null) return true;
            }
        ...
        log(`  Terminal visible: ${terminalVisible}`);
        await shot('14-terminal');
      } catch(e) { log(`  Terminal error: ${e.message.slice(0,80)}`); }
    });

    // ========================================
    // Find/Replace & Search
    // ========================================
    await runTest(14, '14.1', 'Find in file (Ctrl+F) opens search widget', 'P1', async () => {
      try {
        if (!helloContent || !helloContent.includes('HelloServlet')) {
          helloContent = await openFileWithVerification(page, helloPath, 'HelloServlet.java', 'HelloServlet', 2);
        }
        await page.keyboard.press('Control+F');
        await sleep(1000);
        const findWidget = await page.evaluate(() => {
          return !!document.querySelector('.monaco-findInput, .find-widget, .search-view');
        });
        log(`  Find widget visible: ${findWidget}`);
        await page.keyboard.press('Escape');
        await sleep(500);
        await shot('15-find-widget');
      } catch(e) { log(`  Find widget error: ${e.message.slice(0,80)}`); }
    });

    await runTest(15, '15.1', 'Global search (Ctrl+Shift+F) available', 'P1', async () => {
      try {
        await page.keyboard.press('Control+Shift+F');
        await sleep(2000);
        const searchView = await page.evaluate(() => {
          return !!document.querySelector('.search-view, .theia-search-container, #search-in-workspace');
        });
        log(`  Global search view visible: ${searchView}`);
        await page.keyboard.press('Escape');
        await sleep(500);
        await shot('16-global-search');
      } catch(e) { log(`  Global search error: ${e.message.slice(0,80)}`); }
    });

    // ========================================
    // Command Palette & Welcome Page
    // ========================================
    await runTest(2, '2.1', 'Command palette opens via Ctrl+Shift+P', 'P0', async () => {
      await page.keyboard.press('Control+Shift+P');
      await sleep(1000);
      const paletteVisible = await page.evaluate(() => {
        return !!document.querySelector('.quick-input-widget, .monaco-quick-open-widget');
      });
      log(`  Command palette visible: ${paletteVisible}`);
      await page.keyboard.press('Escape');
      await sleep(500);
      await shot('17-command-palette');
      if (!paletteVisible) throw new Error('Command palette not visible');
    });

    await runTest(3, '3.1', 'Welcome page can be opened', 'P1', async () => {
      try {
        await execCmd(page, 'kairo.welcome.show');
        await sleep(2000);
        const welcomeVisible = await page.evaluate(() => {
          const welcomeEl = document.querySelector('.kairo-welcome, .welcome-widget, [id*="welcome"]');
          if (welcomeEl && welcomeEl.offsetParent !== null) return true;
          const body = document.body.textContent || '';
          return /welcome|欢迎|Kairo IDE/i.test(body.slice(0, 5000));
        });
        log(`  Welcome page visible: ${welcomeVisible}`);
        await shot('18-welcome-page');
      } catch(e) { log(`  Welcome page error: ${e.message.slice(0,80)}`); }
    });

    await runTest(0, '0.3', 'Status bar shows project and JDK info', 'P1', async () => {
      const bar = await getStatusBarText(page);
      log(`  Status bar: ${bar.slice(0, 300)}`);
      const checks = {
        project: /Project:|项目:/.test(bar),
        jdk: /JDK:|Java/.test(bar),
        encoding: /UTF-8|GBK|gbk|utf-8/.test(bar),
        agent: /Agent:|已连接|connected/.test(bar),
      };
      log(`  Status indicators: project=${checks.project}, jdk=${checks.jdk}, encoding=${checks.encoding}, agent=${checks.agent}`);
      await shot('19-status-bar-detail');
    });

    // ========================================
    // Additional Editor & UI Tests
    // ========================================
    await runTest(6, '6.6', 'Quick Open (Ctrl+P) searches and opens files', 'P0', async () => {
      log('  Testing Quick Open file search...');
      await page.keyboard.press('Control+P');
      await sleep(1000);
      const widgetVisible = await page.evaluate(() => {
        return !!document.querySelector('.quick-input-widget, .monaco-quick-open-widget');
      });
      log(`  Quick Open widget visible: ${widgetVisible}`);
      if (widgetVisible) {
        await page.keyboard.type('I18nServlet', { delay: 50 });
        await sleep(2000);
        await page.keyboard.press('Enter');
        await sleep(3000);
        const content = await getEditorContent(page);
        const found = content.includes('I18nServlet');
        log(`  Quick Open result contains I18nServlet: ${found} (content: ${content.slice(0,80)}...)`);
        await shot('20-quick-open');
        await page.keyboard.press('Escape');
        await sleep(500);
      }
    });

    await runTest(6, '6.7', 'File save with Ctrl+S works without errors', 'P0', async () => {
      try {
        await page.keyboard.press('Control+S');
        await sleep(1500);
        const saveDialog = await page.evaluate(() => {
          const dialogs = document.querySelectorAll('.dialogBlock, .theia-dialog');
          for (const d of dialogs) { if (d.offsetParent !== null) return true; }
          return false;
        });
        log(`  Save dialog shown: ${saveDialog} (should be false for already-saved file)`);
        if (saveDialog) {
          await page.keyboard.press('Escape');
          await sleep(500);
        }
        await shot('21-file-save');
      } catch(e) { log(`  Save test error: ${e.message.slice(0,80)}`); }
    });

    await runTest(5, '5.2', 'Activity bar switches between views', 'P1', async () => {
      try {
        const activityBar = page.locator('#theia-left-side-bar, .theia-activity-bar').first();
        if (await activityBar.count() > 0) {
          const tabs = activityBar.locator('.p-TabBar-tab, .theia-activity-tab');
          const tabCount = await tabs.count();
          log(`  Activity bar tabs: ${tabCount}`);
          let searchViewFound = false;
          for (let i = 0; i < Math.min(tabCount, 5); i++) {
            await tabs.nth(i).click({ force: true }).catch(() => {});
            await sleep(600);
          }
          await shot('22-activity-bar');
        }
      } catch(e) { log(`  Activity bar test error: ${e.message.slice(0,80)}`); }
    });

    await runTest(13, '13.1', 'Notification center/toasts appear', 'P1', async () => {
      try {
        const notifications = await page.evaluate(() => {
          const notifs = document.querySelectorAll('.theia-notification, .notification-toast, .notifications-center');
          return notifs.length;
        });
        log(`  Notification elements found: ${notifications}`);
        await shot('23-notifications');
      } catch(e) { log(`  Notification test error: ${e.message.slice(0,80)}`); }
    });
...
function generateReport(results, bugs, consoleErrors, logErrors, leftoverProcesses, duration) {
  const pass = results.filter(r => r.status === 'pass').length;
  const lines = [
    '# Kairo IDE Deep E2E Test Report (v8d)', '',
