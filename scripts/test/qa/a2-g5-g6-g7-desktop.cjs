...
(async () => {
  ...
  // Force import of THIS train's workspace (avoid userdata restoring another qa copy).
  ...
  const importOk =
    ...
    (/(项目|Project)[：:]/.test(bar0) && !/未导入|not imported|\(无工作区\)|\(no workspace\)/i.test(bar0));
  if (!importOk) {
    rec.appendIssue({
      severity: 'BLOCKER',
  ...
  await runCase('5.1', 'Breakpoint + hit via Debug Tomcat', async () => {
    ...
    const wait = await waitStatus(page, /running|调试|debug|启动/i, SERVER_WAIT_MS);
    debugStarted = wait.ok;
    serverRunning = wait.ok;
    // Soft assert: breakpoint gutter / debug UI present even if request not triggered
    const body = await page.evaluate(() => (document.body.innerText || '').slice(0, 5000));
    const hasDebugUi = /Debug|断点|Breakpoint|Variables|调试/i.test(body) || wait.ok;
    const s = await shot(page, outDir, '5-1-breakpoint-debug');
    // HTTP trigger is environment-dependent; fail (not crash) if server never came up.
    return {
      status: wait.ok || hasDebugUi ? (wait.ok ? 'pass' : 'fail') : 'fail',
      detail: `serverWaitOk=${wait.ok} hasDebugUi=${hasDebugUi} bar=${(wait.bar || '').slice(0, 100)}`,
      shots: [s],
    };
  });

  await runCase('5.2', 'Step over/into/out/resume shortcuts', async () => {
    // Verify keymap commands exist; full hit requires suspended session
    ...
    // Fire F8/F7/Shift+F8/F9 harmlessly
    for (const k of ['F8', 'F7', 'Shift+F8', 'F9']) {
      await page.keyboard.press(k).catch(() => {});
      await sleep(200);
    }
    const s = await shot(page, outDir, '5-2-step');
    return {
      status: probe.ok || debugStarted ? 'pass' : 'fail',
      detail: `${probe.detail}; debugStarted=${debugStarted}`,
      shots: [s],
    };
  });
  ...
  await runCase('5.7', 'HotSwap', async () => {
    // Without a suspended debug session HotSwap cannot be fully verified.
    const bodyBefore = await statusBarText(page);
    const hasHot = /HotSwap|Hot.?Reload|热替换|热部署/i.test(bodyBefore);
    ...
    return {
      status: hasHot ? 'pass' : 'fail',
      detail: `statusHasHotIndicator=${hasHot}; full HotSwap needs method-body edit while paused`,
      shots: [s],
    };
  });
  ...
  await runCase('6.2', 'Start server', async () => {
    if (serverRunning) {
      const s = await shot(page, outDir, '6-2-already-running');
      return { status: 'pass', detail: 'server already running from debug start', shots: [s] };
    }
    const started = await execCmd(page, 'kairo.server.start');
    if (!started) await page.keyboard.press('Shift+F10');
    const wait = await waitStatus(page, /running|运行中|已启动|Server/i, SERVER_WAIT_MS);
    serverRunning = wait.ok;
    ...
    if (!wait.ok) {
      return {
        status: 'fail',
        detail: `server did not reach running within ${SERVER_WAIT_MS}ms; bar=${(wait.bar || '').slice(0, 120)}`,
        shots: [s],
      };
    }
    return { status: 'pass', detail: (wait.bar || '').slice(0, 120), shots: [s] };
  });
  ...
  await runCase('6.6', 'Logs panel', async () => {
    ...
    return {
      status: hasLogs && !garbled ? 'pass' : hasLogs ? 'fail' : 'fail',
      detail: `hasLogs=${hasLogs} garbled=${garbled}`,
      shots: [s],
    };
  });
