...
const rec = makeRecorder({ agent: 'A8', instance: 'B6', outDir });
...
(async () => {
  ...
  try {
    ...
    await rec.record('2.8', 'Search case/regex/word toggles', async (ctx) => {
      ...
      const base = await page.locator('[data-testid="search-result"]').count();
      await runSearch(page, { query: 'hello', caseSensitive: true });
      const cased = await page.locator('[data-testid="search-result"]').count();
      await runSearch(page, { query: 'Hel+o', regex: true });
      const regexed = await page.locator('[data-testid="search-result"]').count();
      const name = await shot(page, outDir, '2-8-search-toggles');
      ctx.addShot(name);
      await clearOverlays(page);
      return { status: 'pass', detail: `base=${base} case=${cased} regex=${regexed}` };
    });
