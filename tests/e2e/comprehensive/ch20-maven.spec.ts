...
import { openIde, openKairoCommand, uiOrFile, expectContract, api } from './campaign';
...
test.describe.serial('ch20 maven', () => {
  ...
  test('TC-MAVEN-002 empty pom state', async () => {
    expectContract('packages/i18n/src/locales/en.ts', 'No pom.xml found in workspace root.');
  });
  test('TC-MAVEN-003 lifecycle goals', async () => {
    expectContract('packages/theia-product/src/main/browser/maven-view-widget.tsx', 'compile', 'package', 'install');
  });
  ...
  test('TC-MAVEN-005 dependency tree indent', async () => {
    expectContract('packages/theia-product/src/main/browser/maven-view-widget.tsx', 'depth * 16');
  });
  test('TC-MAVEN-006 conflict detection', async () => {
    expectContract('packages/theia-product/src/main/browser/maven-view-widget.tsx', 'maven-conflicts');
  });
  test('TC-MAVEN-007 output pre block', async () => {
    expectContract('packages/theia-product/src/main/browser/maven-view-widget.tsx', 'maven-output');
  });
  test('TC-MAVEN-008 aggregator modules', async () => {
    expectContract('packages/java-extension/src/browser/maven-view-widget.tsx', 'maven-modules');
  });
});
