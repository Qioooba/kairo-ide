...
import { openIde, openKairoCommand, paletteLists, expectContract } from './campaign';
...
test.describe.serial('ch23 debug', () => {
  ...
  test('TC-DBG-002 step commands registered', async () => {
    expectContract('packages/theia-product/src/main/browser/kairo-idea-windows-keymap.ts', 'F8', 'F7');
  });
  test('TC-DBG-003 run to cursor', async () => {
    expectContract('packages/theia-product/src/main/browser/kairo-idea-windows-keymap.ts', 'alt+f9');
  });
  test('TC-DBG-004 stop Ctrl+F2', async () => {
    expectContract('packages/theia-product/src/main/browser/kairo-idea-windows-keymap.ts', 'ctrl+f2');
  });
  ...
  test('TC-DBG-006 conditional breakpoint on-hit eval', async () => {
    ...
    expectContract('runtime-agent/internal/debug/breakpoint.go', 'RecordHitAndShouldStop');
    expectContract('packages/java-extension/src/browser/kairo-java-debug-breakpoint-contribution.ts', 'condition');
  });
  test('TC-DBG-007 hit count', async () => {
    expectContract('runtime-agent/internal/debug/breakpoint.go', 'HitCountMode');
  });
  test('TC-DBG-008 logpoint', async () => {
    expectContract('packages/java-extension/src/browser/kairo-java-debug-breakpoint-contribution.ts', /logpoint|LogMessage/i);
  });
  test('TC-DBG-009 exception breakpoint', async () => {
    expectContract('runtime-agent/internal/debug/exception_breakpoint.go', 'ExceptionBreakpoint');
  });
  ...
  test('TC-DBG-021 Variables view', async () => {
    ...
    expectContract('packages/theia-product/src/main/browser/debug-variables-widget.tsx', 'Variable');
  });
  test('TC-DBG-022 Call Stack view', async () => {
    await openKairoCommand(page, 'Kairo: Show Debug Callstack');
    expectContract('packages/theia-product/src/main/browser/kairo-views-contribution.tsx', 'Show Debug Callstack');
  });
  test('TC-DBG-023 Watch view', async () => {
    await openKairoCommand(page, 'Kairo: Show Debug Watch');
    expectContract('packages/theia-product/src/main/browser/debug-watches-idea.tsx', 'watch');
  });
  test('TC-DBG-024 Debug Console', async () => {
    await openKairoCommand(page, 'Kairo: Show Debug Console');
    expectContract('packages/theia-product/src/main/browser/kairo-views-contribution.tsx', 'Evaluate Expression');
  });
  ...
  test('TC-DBG-026 Debug Toolbar', async () => {
    ...
    expectContract('packages/theia-product/src/main/browser/kairo-views-contribution.tsx', 'Show Debug Toolbar');
  });
  ...
  test('TC-DBG-028 hover evaluate', async () => {
    expectContract('packages/theia-product/src/main/browser/debug-hover-provider.ts', 'hover');
  });
  ...
  test('TC-DBG-042 attach JVM', async () => {
    expectContract('runtime-agent/internal/debug/debug_state.go', 'attach');
  });
  test('TC-DBG-043 source mismatch', async () => {
    expectContract('runtime-agent/internal/debug/debug_state.go', 'StateSuspended');
  });
  test('TC-DBG-044 acceptance state machine', async () => {
    expectContract('runtime-agent/internal/debug/debug_state.go', 'StateRunning');
  });
});
